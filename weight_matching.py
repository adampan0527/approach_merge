...
def resnet20_permutation_spec_custom() -> PermutationSpec:
    conv = lambda name, p_in, p_out: {f"{name}.weight": (p_out, p_in, None, None,)}
...
def vgg16_permutation_spec() -> PermutationSpec:
    ...
    return permutation_spec_from_axes_to_perm({
        ...
        **{f"features.{layers_with_conv[i]}.weight": (
        f"P_Conv_{layers_with_conv[i]}", f"P_Conv_{layers_with_conv_b4[i]}", None, None,)
...
def apply_permutation(ps: PermutationSpec, perm, params):
    ...
    return {k: params[k] if 'num_batches_tracked' in k else get_permuted_param(ps, perm, k, params) for k in params.keys()}
...
def weight_matching(ps: PermutationSpec, params_a, params_b, max_iter=100, init_perm=None):
    ...
    for iteration in range(max_iter):
        progress = False
        # no_perm = torch.arange(start=0, end=len(perm_names))
        # print(no_perm)
        # for p_ix in no_perm:
