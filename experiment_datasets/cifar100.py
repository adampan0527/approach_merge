...
def prepare_train_loaders(config):
    ...
    loaders = {'full': torch.utils.data.DataLoader(train_dset, batch_size=config['batch_size'],
                                                   shuffle=config['shuffle_train'], num_workers=config['num_workers'])}
    ...
    if 'class_splits' in config:
        ...
        for i, splits in enumerate(config['class_splits']):
            ...
            loaders['splits'].append(torch.utils.data.DataLoader(
                data_subset, batch_size=config['batch_size'], shuffle=config['shuffle_train'],
...
def prepare_test_loaders(config):
    ...
    loaders = {
        'full': torch.utils.data.DataLoader(test_dset, batch_size=config['batch_size'], shuffle=config["shuffle_test"],
                                            num_workers=config['num_workers'])}
